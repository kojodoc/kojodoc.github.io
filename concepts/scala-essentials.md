...
* Commands also return output values! It's just that each and every command returns the exact same output value - called unit (written `()`). The type of this unit value is `Unit`. You can think of `()` as meaning *no information*.
