...
Here's the absolute minimum amount of programming theory and Scala information that you need to know - to be able to write games in Kojo.
...
* Each line can contains instructions for the computer. These instructions can be of the following kinds:
  * Commands - which let you take actions that can be sensed in the external world (e.g. `forward(100)`, `println(10)`), or indirectly affect future actions (e.g. `setPenColor(blue)`).
  ...
  * Queries - which let you access (potentially changing) values in the program's environment (e.g. `position`, `heading`, `readInt("Enter a number")`, `random(10`).
...
3. Show you the results - via the use of commands.
4. Optionally go back to step 2.
...
* Every data value is an object. An object combines data with functions/commands. The functions/commands attached to an object are called its methods.
  ...
  * Some primitive types are - Int, Double, String, Color, Sequence, Map, Set, etc.
...
* Sequencing - instructions run one line after the other, top to bottom
* Looping - code that you give to the `animate` command gets called approximately fifty times per second
